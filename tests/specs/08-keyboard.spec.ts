...
test.describe('@fast Keyboard shortcuts', () => {
  ...
  test('Ctrl held flips the send button icon vs current reasoning state', async () => {
    const page = getPage();
  ...
  test('Ctrl+Enter sends a message', async () => {
    const page = getPage();
    const input = page.locator('#input');
  ...
  test('Escape clears input when palette is closed and not loading', async () => {
    const page = getPage();
    const input = page.locator('#input');
  ...
  test('Escape closes command palette (first press only hides palette)', async () => {
    const page = getPage();
    const input = page.locator('#input');
    await input.type('/');
  ...
  test('Escape closes context inspector modal', async () => {
    const page = getPage();
  ...
  test('Escape closes settings modal', async () => {
    const page = getPage();
  ...
  test('Tab completes a palette selection and submits it', async () => {
    const page = getPage();
    const input = page.locator('#input');
