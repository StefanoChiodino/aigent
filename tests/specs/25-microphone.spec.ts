 ...
 * ScriptProcessorNode into the page. This allows us to drive the mic
 * lifecycle without actual hardware.
 *
 * Returns an object on window.__micMock with controls:
 *   - fireAudioFrame(rms): simulate an onaudioprocess event with the given RMS level
 *   - stopTracks(): simulate media stream track stop
 *   - getState(): returns { processorCreated, sourceConnected, processorConnected }
 */
async function installMicMock(page: import('@playwright/test').Page) {
  await page.evaluate(() => {
    ...
    window.AudioContext = class MockAudioContext {
      ...
      createOscillator() {
        // For playMicSound — return a minimal mock
        return {
          connect: () => {},
    ...
    window.__micMock = {
      state: mockState,
      mockProcessor,
      /** Simulate an audio frame with a given RMS level */
      fireAudioFrame(rms: number) {
        const handler = mockProcessor.onaudioprocess;
        if (!handler) return;
        // Create a buffer that produces the desired RMS
        // RMS = sqrt(sum(x^2)/N), so for uniform value: x = rms
...
async function mockSTTDelayed(page: import('@playwright/test').Page, text: string, delayMs: number) {
...
test.describe('Microphone UI state, controls, and VAD feedback', () => {
