...
import { waitForConnected, expectHidden } from '../helpers/ui.js';
...
test('error event shows the error bar', async ({ page }) => {
...
test('error bar displays the error message', async ({ page }) => {
...
test('error bar is hidden initially', async ({ page }) => {
  await expectHidden(page.locator('#error-bar'));
});

test('error bar is cleared when a new error event has empty message', async ({ page }) => {
...
test('loading event sets data-working attribute on body', async ({ page }) => {
...
test('loading false removes data-working attribute', async ({ page }) => {
  await injectEvent({ type: 'loading', isLoading: true });
  await expect(page.locator('body')).toHaveAttribute('data-working', { timeout: 3_000 });
  await injectEvent({ type: 'loading', isLoading: false });
...
test('cancel button is visible while loading', async ({ page }) => {
  await injectEvent({ type: 'loading', isLoading: true });
...
test('system event appends a system message to the chat', async ({ page }) => {
...
test('multiple system messages all appear in the chat', async ({ page }) => {
