...
test('@ palette shows Files section when mounts are active and files returned', async ({ page }) => {
...
test('@ palette shows file path in description', async ({ page }) => {
  await page.route('**/files**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
...
test('selecting a file item inserts its container path into the input', async ({ page }) => {
  await page.route('**/files**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ files: [{ path: 'src/agent.ts', mountPath: '/workspace' }] }),
...
test('@ file search passes query to /files endpoint', async ({ page }) => {
...
test('@ palette with no mounts does not fetch /files', async ({ page }) => {
  // Ensure no mounts
...
test('@ palette shows multiple file results', async ({ page }) => {
  await page.route('**/files**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({
...
test('ArrowDown navigates into file results', async ({ page }) => {
  await page.route('**/files**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
...
test('Escape closes palette and preserves @ query', async ({ page }) => {
  await page.route('**/files**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
