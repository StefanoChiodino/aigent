...
export const SETTINGS_SCHEMA: SettingDef[] = [
  ...
  },
  {
    ...
    desc: 'Volume level (RMS) below which audio is treated as silence. Raise this if background noise or breathing is triggering the mic — the higher the value, the louder you need to be for speech to register. Default 0.015 works in a quiet room; try 0.025–0.04 in a noisier environment.',
    group: 'Microphone',
    type: 'number',
    default: 0.015,
    min: 0.001,
  ...
  },
  {
    ...
    desc: 'How many consecutive loud frames must occur before speech is considered active. Each frame is ~256 ms. Raise this to filter out brief noise spikes (a tap, a click, a short breath) that cross the volume threshold but are too short to be real speech. Default 2 (~0.5 s); try 3–4 if single sounds keep triggering the mic.',
    group: 'Microphone',
    type: 'number',
    default: 2,
    min: 1,
